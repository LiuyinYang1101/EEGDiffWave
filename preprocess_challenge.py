...
def get_data(path, subject, LOSO = False, isStandard = True):
    ...
    y_train_onehot = torch.nn.functional.one_hot(torch.from_numpy(y_train_onehot).to(torch.int64))
    # Prepare testing data 
    N_test, N_ch, _ = X_test.shape 
    X_test = X_test[:, :, t1:t2].reshape(N_test, 1, N_ch, T)
    y_test_onehot = (y_test-1).astype(int)
    y_test_onehot = torch.nn.functional.one_hot(torch.from_numpy(y_test_onehot).to(torch.int64))
