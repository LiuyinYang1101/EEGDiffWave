...
class Residual_block(nn.Module):
    def __init__(self, res_channels, skip_channels, dilation,
        ...
        self.eeg_spatial_channel_down = nn.Conv2d(1, 1, (22, 1), bias = False)  # spatial filter 22channel=>1channel
        self.eeg_spatial_channel_up = ZeroConv1d(1,22)  # spatial filter 1channel=>22channel
        # dilated conv layer
        ...
        self.res_conv = nn.Conv1d(1, 22, kernel_size=1)
        self.res_conv = nn.utils.weight_norm(self.res_conv)
        nn.init.kaiming_normal_(self.res_conv.weight)

        # skip conv1x1 layer, add to all skip outputs through skip connections
        self.skip_conv = nn.Conv1d(1, 22, kernel_size=1)
    ...
    def forward(self, input_data):
        ...
        h = h.unsqueeze(1)
        #print(h.shape)
        h = self.eeg_spatial_channel_down(h)
        h = h.squeeze(1)
        B, C, L = h.shape
        #print("B after", B)
        #print("C after", C)
    ...
    def forward(self, input_data):
        ...
        for n in range(self.num_res_layers):
            h, skip_n = self.residual_blocks[n](
                (h, mel_spectrogram, diffusion_step_embed))  # use the output from last residual layer
...
class WaveNet_vocoder(nn.Module):
    def __init__(self, in_channels, res_channels, skip_channels, out_channels,
        ...
        self.final_conv = nn.Sequential(Conv(22, 22, kernel_size=1),
                                        nn.ReLU(),
                                        ZeroConv1d(22, 22))
